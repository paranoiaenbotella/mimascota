...
class Animal extends Modelo
{
    ...
    {
        $consulta = $this->conexion->prepare(
            "INSERT INTO `animales` (`id_animales_tipos`, `id_usuarios`,  `nombre`) VALUES (?, ?, ?)"
        );
    ...
    public function listarAnimalesPorIdUsuario($idUsuario)
    {
        $animales = [];
        $consulta = $this->conexion->prepare("SELECT * FROM `animales` WHERE `id_usuarios` = ? ORDER BY `id`");

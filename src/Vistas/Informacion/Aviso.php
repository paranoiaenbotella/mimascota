<!--AVISO LEGAL-->
<section class="container">
  <div class="row">
    <div class="col-sm-12  col-md-12 col-lg-12 mt-3">
    	<div class="row">
    <div class="col-sm-10 offset-sm-1 col-md-10 offset-md-1 col-lg-10 offset-lg-1 mt-3">
    <h3> Condiciones de uso</h3>	
    <p>Usted utiliza el sitio Web en la condición de Usuario lo que implica
    la aceptación completa de las condiciones y cláusulas incluidos en este documento.</p>
    <p>Tiene la obligación de no utilizar el contenido del sitio Web de una forma que pueda dañar, inutilizar o impedir la normal utilización de los contenidos y equipos informáticos del propietario del sitio Web o de cualquier usuario de Internet.</p>
    <h4> Opiniones</h4>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vitae turpis placerat, molestie justo et, porta dui. Donec quam ante, blandit et ipsum faucibus, commodo tempus risus. Maecenas eu orci tellus. Integer porta semper massa sit amet sagittis. Duis mollis leo id porta molestie. Nullam finibus, justo quis semper gravida, lorem ligula mattis est, eu sagittis quam diam vel odio. Integer sed ante sed mi elementum porttitor.
    </p>
    <h4>Medidas de seguridad</h4>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vitae turpis placerat, molestie justo et, porta dui. Donec quam ante, blandit et ipsum faucibus, commodo tempus risus. Maecenas eu orci tellus. Integer porta semper massa sit amet sagittis. Duis mollis leo id porta molestie. Nullam finibus, justo quis semper gravida, lorem ligula mattis est, eu sagittis quam diam vel odio. Integer sed ante sed mi elementum porttitor.
    </p>
    <h4>Contenidos</h4>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vitae turpis placerat, molestie justo et, porta dui. Donec quam ante, blandit et ipsum faucibus, commodo tempus risus. Maecenas eu orci tellus. Integer porta semper massa sit amet sagittis. Duis mollis leo id porta molestie. Nullam finibus, justo quis semper gravida, lorem ligula mattis est, eu sagittis quam diam vel odio. Integer sed ante sed mi elementum porttitor.</p>
		<h4>Propiedad intelectual e industrial</h4>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vitae turpis placerat, molestie justo et, porta dui. Donec quam ante, blandit et ipsum faucibus, commodo tempus risus. Maecenas eu orci tellus. Integer porta semper massa sit amet sagittis. Duis mollis leo id porta molestie. Nullam finibus, justo quis semper gravida, lorem ligula mattis est, eu sagittis quam diam vel odio. Integer sed ante sed mi elementum porttitor.</p>
		<h4>Limitación de responsabilidad</h4>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vitae turpis placerat, molestie justo et, porta dui. Donec quam ante, blandit et ipsum faucibus, commodo tempus risus. Maecenas eu orci tellus. Integer porta semper massa sit amet sagittis. Duis mollis leo id porta molestie. Nullam finibus, justo quis semper gravida, lorem ligula mattis est, eu sagittis quam diam vel odio. Integer sed ante sed mi elementum porttitor.</p>
    </div>
  </div>
    </div>
  </div>
</section>